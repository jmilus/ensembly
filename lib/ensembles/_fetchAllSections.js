...
const getAllSections = async (data) => {
	const { ensembleTypeId } = data;
	const fetchedSections = await prisma.section.findMany({
		where: {
			ensembleTypeId: ensembleTypeId
		}
	})
    //return fetchedSections;
    // const processedEnsembles = fetchedSections.map(ensemble => {
